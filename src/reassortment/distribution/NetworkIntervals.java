...
/**
 * changed by Nicola Felix Mueller to allow for network input
 *
...
public class NetworkIntervals extends CalculationNode implements IntervalList {
    public Input<Network> networkInput = new Input<Network>("network", "tree for which to calculate the intervals", Validate.REQUIRED);
    ...
    protected NetworkIntervalType[] networkIntervalType;
     ...
     */
    protected List<Networknode>[] lineagesAdded;
    protected List<Networknode>[] lineagesRemoved;
    
    // Added these so can restore when needed for structured models
    protected List<Networknode>[] storedLineagesAdded;
    protected List<Networknode>[] storedLineagesRemoved;
    ...
    public NetworkIntervals() {
    }

    public NetworkIntervals(Network network) {
    	calculateIntervals();
    }
...
//        setIntervalsClean();
    ...
    @SuppressWarnings("unchecked")
	public List<Networknode>[] deepCopyLineagesAdded() {
    	List<Networknode>[] newList = new List[intervalCount];
    	for (int i = 0; i < intervalCount; i++) {
    ...
    @SuppressWarnings("unchecked")
	public List<Networknode>[] deepCopyLineagesRemoved() {    	
    	List<Networknode>[] newList = new List[intervals.length];
    	for (int i = 0; i < intervalCount; i++) {
    ...
    public int getSampleCount() {
        ...
        return network.getInternalNodeCount();
    }
    ...
    public List<Networknode> getLineagesAdded(int i) {
        if (!intervalsKnown) {
            calculateIntervals();
        }
        if (i >= intervalCount) throw new IllegalArgumentException();
    ...
    public List<Networknode> getLineagesRemoved(int index) {
        if (!intervalsKnown) {
            calculateIntervals();
        }
     ...
     */
    public NetworkIntervalType getNetworkIntervalType(int i) {
        if (!intervalsKnown) {
            calculateIntervals();
        }
        if (i >= intervalCount) throw new IllegalArgumentException();
        
//        System.out.println(Arrays.toString(networkIntervalType));
        return networkIntervalType[i];
        
//        int numEvents = getCoalescentEvents(i);
//
//        if (numEvents > 0) return NetworkIntervalType.COALESCENT;
//        else if (numEvents < 0) return NetworkIntervalType.SAMPLE;
//        else return NetworkIntervalType.NOTHING;
    }
    ...
    protected void setIntervalsClean(){
        intervalIsDirty = new boolean[networkInput.get().getNodeCount()];
        lastIntervalDirty = false;
    }


    /**
     * Recalculates all the intervals for the given network.
     */
    int count = 0;
    @SuppressWarnings("unchecked")
...
//        int[] parentCounts = new int[nodeCount];
        Integer[] reassNumber = new Integer[nodeCount];
        

        collectTimes(network, times, childCounts, reassNumber);
        ...
        if (intervals == null || intervals.length != nodeCount) {
            ...
            networkIntervalType = new NetworkIntervalType[nodeCount];
            ...
            for (List<Networknode> l : lineagesAdded) {
            	if (l != null) l.clear();
            }
            for (List<Networknode> l : lineagesRemoved) {
            	if (l != null) l.clear();
            }
        }
        ...
        while (nodeNo < nodeCount) {
            ...
            do {
                ...
                final Integer rNumber = reassNumber[childIndex];
               
                
                // don't use nodeNo from here on in do loop
                nodeNo += 1;
                if (rNumber != null){
                    final Networknode parent = network.getNode(childIndex);
                    if (childCount == 1) {
	                    // record removed lineages	
	                	removeLineage(intervalCount, parent.getLeft());
	                    addLineage(intervalCount, parent);
	                    networkIntervalType[intervalCount] = NetworkIntervalType.REASSORTMENTSTART;
	                    lineagesRemoved += (childCount - 1);
	                    lineagesAdded += 1;
                    }else{
                		addLineage(intervalCount, parent);
	                    networkIntervalType[intervalCount] = NetworkIntervalType.REASSORTMENTEND;
	                    lineagesAdded += 1;
                	}                	
                }else if (childCount == 0) {
                    addLineage(intervalCount, network.getNode(childIndex));
                    lineagesAdded += 1;
                    networkIntervalType[intervalCount] = NetworkIntervalType.SAMPLE;
               } else {
                    lineagesRemoved += (childCount - 1);
                    ...
                    for (int j = 0; j < childCount; j++) {
                    	Networknode child = j == 0 ? parent.getLeft() : parent.getRight();
                    ...
                    networkIntervalType[intervalCount] = NetworkIntervalType.COALESCENT;
               }
...
//        print();
    ...
    protected void addLineage(int interval, Networknode node) {
        if (lineagesAdded[interval] == null) lineagesAdded[interval] = new ArrayList<Networknode>();
        lineagesAdded[interval].add(node);
    }

    protected void removeLineage(int interval, Networknode node) {
        if (lineagesRemoved[interval] == null) lineagesRemoved[interval] = new ArrayList<Networknode>();
     ...
     */
    protected static void collectTimes(Network network, double[] times, int[] childCounts, Integer[] reassortmentNumber) {
    	Networknode[] nodes = network.getNodesAsArray();
        for (int i = 0; i < nodes.length; i++) {
        	Networknode node = nodes[i];
            times[i] = node.getHeight();
            childCounts[i] = node.getChildCount();
            reassortmentNumber[i] = node.getReassortmentNumber();
        }
    }
    ...
    public double[] getIntervalStartEnd(int i){
    	...
    	if (i == 0){
    		...
    		List<Networknode> addedNodes = getLineagesAdded(i-1);
    		times[0] = addedNodes.get(0).getHeight();
    	}
		List<Networknode> addedNodes = getLineagesAdded(i);
...
//        		List<Networknode> incomingLines = getLineagesAdded(i);
...
//        		List<Networknode> daughter = getLineagesRemoved(i);
//            	
//            	if (daughter.size() > 2) {
...
//        		     * The lineages in each interval (stored by node ref).
//        		     */
//        		    List<Networknode> lineageAdded = lineagesAdded[j];
//        		    lineagesAdded[j] = lineagesAdded[i];
//        		    lineagesAdded[i] = lineageAdded;
//        		    List<Networknode> lineageRemoved = lineagesRemoved[j];
...
//
	...
	public IntervalType getIntervalType(int i) {
		// TODO just needed to avoid conflicts with the IntevalType
		return null;
	}

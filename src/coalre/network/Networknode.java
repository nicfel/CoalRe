...
public class Networknode extends BEASTObject {
    ...
    public void setHeight(final double height) {
        ...
        if (!isLeaf()) {
            getLeft().isDirty |= Network.IS_DIRTY;
            if (getRight() != null) {
                getRight().isDirty |= Network.IS_DIRTY;
            }
        }
    }
     ...
     */
    public void setParent(final Networknode parent) {
     ...
     */
    void setParent(final Networknode parent, final boolean inOperator) {
        if (inOperator) startEditing();
        if (this.parent != parent) {
        	this.parent = parent;
            if (inOperator) isDirty = Network.IS_FILTHY;
        }
    }
    ...
    public void setSecondParent(final Networknode parent) {
        setSecondParent(parent, true);
    }

    void setSecondParent(final Networknode parent, final boolean inOperator) {
        if (inOperator) startEditing();
        if (this.secondparent != parent) {
     ...
     * @param parent     the node to become parent
     */
    void setParentImmediate(final Networknode parent) {
        this.parent = parent;
    }
    ...
    public int getNodeCount() {
        int nodes = 1;
        if (reassortmentNumber!=null){
//        	System.out.println(children.get(0).getNodeCount());
...
//        	System.out.println(child.getNodeCount());
    ...
    public String toShortNewick(final boolean printInternalNodeNumbers) {
        ...
        if (!isLeaf()) {
            ...
            for (Networknode child : getChildren()) {
                if (isFirst)
                    isFirst = false;
                else
                    buf.append(",");
        ...
        if (reassortmentNumber!=null){
        	buf.append("#").append(getReassortmentNumber());
        }
    ...
    public int sort() {
        ...
        final List<Networknode> children = new ArrayList<>(getChildren());
    ...
    public Networknode copy() {
        ...
        node.reassortmentNumber = reassortmentNumber;
        node.hasSegments = hasSegments;
        ...
        for (final Networknode child : getChildren()) {
    ...
    public void assignFrom(final Networknode[] nodes, final Networknode node) {
        ...
        reassortmentNumber = node.reassortmentNumber;
        hasSegments = node.hasSegments;
...
//    }
     ...
     */
    public void setChild(final int childIndex, final Networknode node) {
     ...
     */
    @Deprecated
    public void setLeft(final Networknode leftChild) {
        if (children.size() == 0) {
     ...
     */
    @Deprecated
    public void setRight(final Networknode rightChild) {
    ...
    public static Networknode connect(final Networknode left, final Networknode right, final double h) {
        final Networknode n = new Networknode();
        n.setHeight(h);
        n.setLeft(left);
        n.setRight(right);
        left.parent = n;
    ...
    public Networknode getDirectAncestorChild() {
        if (!this.isFake()) {
            return null;
        }
    ...
    public Networknode getNonDirectAncestorChild(){
        if (!this.isFake()) {
            return null;
        }
        if ((this.getLeft()).isDirectAncestor()){
    ...
    public Networknode getFakeChild(){

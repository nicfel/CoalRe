...
public class Network extends StateNode implements NetworkInterface {
    final public Input<Network> m_initial = new Input<>("initial", "tree to start with");
    ...
    final public Input<String> nodeTypeInput = new Input<>("nodetype",
            "type of the nodes in the beast.tree", Networknode.class.getName());
     ...
     */
    protected Networknode[] m_nodes = null;

    protected Networknode[] m_storedNodes = null;
...
//            leafNodeCount = other.leafNodeCount;
    ...
    protected void processTraits(List<TraitSet> traitList) {
        for (TraitSet traitSet : traitList) {
            for (Networknode node : getExternalNodes()) {
    ...
    protected void initArrays() {
        ...
        m_nodes = new Networknode[nodeCount];
        listNodes(root, m_nodes);
        m_storedNodes = new Networknode[nodeCount];
     ...
     */
//    public Network(final String newick) {
...
//    }
    ...
    protected void adjustTreeNodeHeights(final Networknode node) {
        if (!node.isLeaf()) {
     ...
     */
    @Override
	public List<Networknode> getExternalNodes() {
        final ArrayList<Networknode> externalNodes = new ArrayList<>();
	...
	public List<Networknode> getInternalNodes() {
        final ArrayList<Networknode> internalNodes = new ArrayList<>();
        for (int i = 0; i < getNodeCount(); i++) {
            final Networknode node = getNode(i);
    ...
    public void setRoot(final Networknode root) {
        ...
        if (m_nodes != null && root.labelNr != m_nodes.length - 1) {
            ...
            Networknode tmp = m_nodes[rootPos];
     ...
     */
    public void setRootOnly(final Networknode root) {
    ...
    @Override
	public Networknode getNode(final int nodeNr) {
    ...
    void collectTaxaNames(final Networknode node) {
        if (node.getID() != null) {
     ...
     */
    @Override
	public void getMetaData(final Networknode node, final Double[] t, final String pattern) {
     ...
     * @param pattern the name of the meta data
     */
    public void getMetaData(final Networknode node, final Integer[] t, final String pattern) {
     ...
     */
    @Override
	public void setMetaData(final Networknode node, final Double[] t, final String pattern) {
...
//    }
    ...
    protected Networknode[] postCache = null;
    @Override
	public Networknode[] listNodesPostOrder(Networknode node, Networknode[] nodes) {
        if( node != null ) {
            return NetworkInterface.super.listNodesPostOrder(node, nodes);
        }
        if( postCache == null ) {
            postCache = NetworkInterface.super.listNodesPostOrder(node, nodes);
        }
        return postCache;
    }
    ...
    public void assignTo(final StateNode other) {
        final Network tree = (Network) other;
    ...
    public void assignFrom(final StateNode other) {
        final Network tree = (Network) other;
        final Networknode[] nodes = new Networknode[tree.getNodeCount()];//tree.getNodesAsArray();
    ...
    public void assignFromFragile(final StateNode other) {
        ...
        final Networknode[] otherNodes = tree.m_nodes;
...
//    	Networknode tree = (Networknode) getCurrent();
...
//    }
    ...
    protected void store() {
        ...
        if (m_storedNodes.length != nodeCount) {
            final Networknode[] tmp = new Networknode[nodeCount];
    ...
    private void storeNodes(final int start, final int end) {
        ...
        for (int i = start; i < end; i++) {
            final Networknode sink = m_storedNodes[i];
            final Networknode src = m_nodes[i];
            sink.height = src.height;
            ...
            final List<Networknode> children = sink.children;
            final List<Networknode> srcChildren = src.children;

            if( children.size() == srcChildren.size() ) {
               // shave some more time by avoiding list clear and add
               for (int k = 0; k < children.size(); ++k) {
                   final Networknode srcChild = srcChildren.get(k);
                   // don't call addChild, which calls  setParent(..., true);
                   final Networknode c = m_storedNodes[srcChild.getNr()];
                   c.parent = sink;
                   children.set(k, c);
               }
            } else {
                ...
                for (final Networknode srcChild : srcChildren) {
                    // don't call addChild, which calls  setParent(..., true);
                    final Networknode c = m_storedNodes[srcChild.getNr()];
                    c.parent = sink;
                    children.add(c);
    ...
    public void restore() {
        ...
        final Networknode[] tmp = m_storedNodes;
     ...
     */
    public String getTaxonId(final Networknode node) {
     ...
     * Use with care!
     */
    public void addNode(final Networknode newNode) {
        final Networknode[] tmp = new Networknode[nodeCount + 1];
    ...
    public boolean childrenChanged(int nodeNr) {
    	Networknode node = m_nodes[nodeNr];
    	Networknode old = m_storedNodes[nodeNr];

 ...
 */
public class CoReassortment extends ReassortmentAnnotator {

	private enum SummaryStrategy {
		MEAN, MEDIAN
	}
	...
	private static class NetworkAnnotatorOptions {
		...
		public String toString() {
			return "Active options:\n" + "Input file: " + inFile + "\n" + "Output file: " + outFile + "\n"
					+ "Burn-in percentage: " + burninPercentage + "%\n" + "Node height and conv. site summary: "
					+ summaryStrategy;
		}
	}

	public CoReassortment(NetworkAnnotatorOptions options) throws IOException {

		// Display options:
		System.out.println(options + "\n");

		// Initialise reader
		ReassortmentLogReader logReader = new ReassortmentLogReader(options.inFile, options.burninPercentage);

		System.out.println(logReader.getNetworkCount() + " Networks in file.");

		System.out.println("The first " + logReader.getBurnin() + " (" + options.burninPercentage
				+ "%) ACGs will be discarded " + "to account for burnin.");
	...
	private void computeReassortmenDistance(Network network, PrintStream ps, double minTipDistance) {
		...
		List<NetworkEdge> sourceEdges = network.getEdges().stream().filter(e -> !e.isRootEdge())
				.filter(e -> e.parentNode.isReassortment()).collect(Collectors.toList());
	...
	private static boolean getOptionsGUI(NetworkAnnotatorOptions options) {
		...
		layout.setHorizontalGroup(layout.createSequentialGroup()
				.addGroup(layout.createParallelGroup().addComponent(logFileLabel).addComponent(outFileLabel)
						.addComponent(burninLabel).addComponent(summaryMethodLabel).addComponent(thresholdLabel)
						.addComponent(geneFlowCheckBox))
				.addGroup(layout.createParallelGroup(GroupLayout.Alignment.LEADING, false).addComponent(inFilename)
						.addComponent(outFilename).addComponent(burninSlider).addComponent(heightMethodCombo)
						.addComponent(thresholdSlider).addComponent(gfOutFilename))
				.addGroup(layout.createParallelGroup(GroupLayout.Alignment.LEADING, false).addComponent(inFileButton)
						.addComponent(outFileButton).addComponent(gfOutFileButton)));

		layout.setVerticalGroup(layout.createSequentialGroup()
				.addGroup(layout.createParallelGroup().addComponent(logFileLabel)
						.addComponent(inFilename, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE,
								GroupLayout.PREFERRED_SIZE)
						.addComponent(inFileButton))
				.addGroup(layout.createParallelGroup().addComponent(outFileLabel)
						.addComponent(outFilename, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE,
								GroupLayout.PREFERRED_SIZE)
						.addComponent(outFileButton))
				.addGroup(layout.createParallelGroup().addComponent(burninLabel).addComponent(burninSlider,
						GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE))
				.addGroup(layout.createParallelGroup().addComponent(summaryMethodLabel).addComponent(heightMethodCombo,
						GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE))
				.addGroup(layout.createParallelGroup().addComponent(thresholdLabel).addComponent(thresholdSlider,
						GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE))
				.addGroup(layout.createParallelGroup().addComponent(geneFlowCheckBox).addComponent(gfOutFilename)
	...
	/**
	 * Prepare JFrame to which ACGAnnotator output streams will be directed.
	 */
	...
	public static String helpMessage = "ACGAnnotator - produces summaries of Bacter ACG log files.\n" + "\n"
	...
	public static void getCLIOptions(String[] args, NetworkAnnotatorOptions options) {
		...
		while (args[i].startsWith("-")) {
			...
			case "-threshold":
				...
				try {
					options.convSupportThresh = Double.parseDouble(args[i + 1]);
				} catch (NumberFormatException e) {
			...
			case "-removeSegments":
				...
				try {
					...
					printUsageAndError(
							"removeSegments must be an array of integers separated by commas if more than one");
				}
	...
	/**
	 * Main method for ACGAnnotator. Sets up GUI if needed then uses the
	 * ACGAnnotator constructor to actually perform the analysis.
	 *
	 * @param args command line arguments
	 */

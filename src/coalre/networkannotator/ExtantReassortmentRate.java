...
/**
 * Computes the reassortment rates for external branches 
 * @author Nicola Felix Müller <nicola.felix.mueller@gmail.com>
 */
public class ExtantReassortmentRate extends ReassortmentAnnotator {
    ...
    public ExtantReassortmentRate(NetworkAnnotatorOptions options) throws IOException {
    ...
    private void computeReassortmentLeaveDist(Network network, PrintStream ps, double minTipDistance){    	
		...
		for (NetworkEdge e : allEdges){
			double tipHeight = e.childNode.getHeight();
			getParentEdgeAndNodes(e, minTipDistance,tipHeight);		
        }
	...
	private void getParentEdgeAndNodes(NetworkEdge edge, double maxTipDistance, double tipHeight) {
    	
    	NetworkNode parent = edge.parentNode;
        
    	// check if the edge crosses the maxTipDistance
    	if (parent.getHeight()>= tipHeight + maxTipDistance) {
			// only count the part of the edge that is above the minTipDistance
    		double usedHeight = edge.childNode.getHeight() - tipHeight;
			double correctedLength = edge.getLength() - usedHeight;
			totalLength += correctedLength*(1.0-Math.pow(0.5, edge.hasSegments.cardinality()-1));
			return;
		}else {					
			totalLength += edge.getLength()*(1.0-Math.pow(0.5, edge.hasSegments.cardinality()-1));
		}
    	

    	
		if (parent.isReassortment()) {
			totalReassortments++;
			for (NetworkEdge parentEdge : parent.getParentEdges()) {
				getParentEdgeAndNodes(parentEdge, maxTipDistance, tipHeight);
			}
		}
    ...
    public static void main(String[] args) {
        ...
        // Run ACGAnnotator
        try {
            new ExtantReassortmentRate(options);
